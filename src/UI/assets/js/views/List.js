...
function (_, $, Backbone, ListItemView, ListItemModel, ListCollection) {
	...
	var ListView = Backbone.View.extend({
		...
		initialize: function(options) {
			...
			if (this.status.get('isChildList_v2') && this.parentList && this.parentModel) {
				...
				this.collection = new ListCollection(data);
			}
			...
			// This is a child list
				...
				this.collection = new ListCollection(contextData);
		...
		addNewModel: function(data) {
			...
			this.collection.add(new ListItemModel(data));
		},
      ...
      $.each(this.$form.find('[bebop-ui-el--postsearch]'), function(index, item) {
	...
	ListView.addFormAction = function(name, fn) {
  ...
  // Associated Types
  ListView.addFormAction('bebop-ui-action--addAssociatedType', function(event) {
  ...
  // Sections
  ListView.addFormAction('bebop-ui-action--addSection', function(event) {
  ...
  // Multi Source Media
  ListView.addFormAction('bebop-ui-action--addMediaFromTargetSource', function(event) {
